...
LOOKAHEAD_DISTANCE_STRAIGHT = 6.5 # meters   # 6.5
...
VELOCITY_STRAIGHT = 6.5 # m/s                # 6.5
VELOCITY_TURN = 0.7                      # 2.0
...
WP_TURN_WINDOW_MIN = 5.5                          # 5.0
WP_TURN_WINDOW_MAX = 6.0                          # 5.5
...
def get_index_of_closest_point(car_pos, path_points):
    closest_index = 0
...
def update_current_index(car_pos, path_points):
    global current_index
    
    max_index = np.clip(current_index + CURRENT_INDEX_FORWARD_THRESHOLD, 0, len(path_points) - 1)
    
    closest_index = 0
    closest_dist = float('inf')
    for index, point in enumerate(path_points[current_index:max_index]):
        dist_to_point = dist(car_pos, point)
        if dist_to_point <= closest_dist:
            closest_dist = dist_to_point
            closest_index = index
...
def get_avg_x_wrt_car(car_pos, path_points, yaw):
    ...
    avg_x_start_index = np.clip(avg_x_start_index, 0, len(path_points) - 1) 
    
    print "avg x start index: ", avg_x_start_index
    
    # index here starts at 0
    for index, point in enumerate(path_points[avg_x_start_index:]):
...
def callback(data):
    ...
    if not found_closest_point:
        current_index = get_index_of_closest_point(car_pos, path_points)
    ...
    avg_x_wrt_car = np.clip(abs(get_avg_x_wrt_car(car_pos, path_points, yaw)), 0.0, MAX_WP_X_OFFSET)
...
#        lookahead_distance = LOOKAHEAD_DISTANCE_STRAIGHT
    ...
    for index, point in enumerate(path_points[current_index:]):
        # Calculate distance between (x,y) and (point_x, point_y)
        dist_to_point = dist(car_pos, point)
    ...
    if current_index > 1500:
        pdb.set_trace()
    
    update_current_index(car_pos, path_points)
